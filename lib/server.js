...
function checkCredentials(callback) {
  ...
  request.get({
    url: url,
    headers: HEADERS,
    'auth': {user: global.user, pass: global.pwd}
...
app.post('/', function(req, res) {
  ...
  case 'issue_comment':
    ...
    function maybeEnqueueJob(payload, pull, event) {
      pull.head.repo.git_url = pull.head.repo.git_url.replace(/^git:\/\//, 'https://'+global.user+':'+global.pwd+'@');
      ...
      var id = randomHash(15),
          jobInfo = {
            ...
            base_url: 'https://'+global.user+':'+global.pwd+'@github.com/'+config.repo+'.git',
